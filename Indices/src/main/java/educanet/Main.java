...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        String maze = "101111111000111011101010000001\n"+
        ...
        String[] oddeleniRadkuTextu = maze.split("\n");

        int MAZEULTRA = oddeleniRadkuTextu.length;

        float bozeProsimUzNikdyMaze = 2 / (float) MAZEULTRA;

        char[][] DataTextaku = new char[MAZEULTRA][MAZEULTRA];
        for (int i = 0; i < MAZEULTRA; i++) {
            for (int j = 0; j < MAZEULTRA; j++) {
                DataTextaku[i][j] = oddeleniRadkuTextu[i].charAt(j);
            }
        }
        GLFW.glfwInit();
        ...
        if (window == 0) {
            ...
            throw new Exception("HAHA CHYBICKA LMFAOOO MAS 0 Z MAZE XDXDXD");
        }
        ...
        Square[][] squares = new Square[MAZEULTRA][MAZEULTRA];

        for (int i = 0; i < DataTextaku.length; i++) {
            for (int j = 0; j < MAZEULTRA; j++) {
                if (DataTextaku[i][j] == '1') {
                    squares[i][j] = new Square((j * bozeProsimUzNikdyMaze-1), 1 - (i*bozeProsimUzNikdyMaze),bozeProsimUzNikdyMaze);
                } else {
                    squares[i][j] = null;
                }
            }
